    ...
    if (isset($_FILES['file']['tmp_name']) && !empty($_FILES['file']['tmp_name'])) {
        ...
        if ($file_ext === 'xlsx' || $file_ext === 'xls') {
            ...
            if (move_uploaded_file($file_tmp, $destinationPath)) {
                ...
                $generalPropertiesColumns = ['B', 'C', 'D', 'E', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'S', 'T', 'Y'];
                $areCsGenPropertiesColumns = ['A', 'P', 'Q', 'R', 'U', 'V', 'W', 'X', 'Z', 'AA', 'AB', 'AD', 'AE'];
                ...
                $generalPropertiesSql = "INSERT INTO generalproperties (article, brand, serialNo, particulars, eNGAS, acquisitionDate, acquisitionCost, propertyNo, accountNumber, estimatedLife, unitOfMeasure, unitValue, quantity, officeName, accountablePerson, gpremarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $AreIcsGenPropertiesSql = "INSERT INTO are_ics_gen_properties (propertyID, dateReceived, onhandPerCount, soQty, soValue, previousCondition, location, currentCondition, dateOfPhysicalInventory, supplier, POnumber, AIRNumber, notes, jevNo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                foreach ($generalPropertiesData as $generalRow) {
                    ...
                    $stmtAreCsGenProperties->bind_param('isssssssssssss', $propertyID, ...$AreIcsGenPropertiesRow);
